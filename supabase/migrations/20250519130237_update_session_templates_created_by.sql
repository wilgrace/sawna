-- Drop all policies on the bookings, session_instances, and session_templates tables
DO $$ 
DECLARE
    policy_name text;
BEGIN
...
-- Disable RLS on session_templates
...
CREATE POLICY "Admins can delete bookings for their session instances on table"
ON public.bookings
AS PERMISSIVE
FOR DELETE
TO authenticated
